          ...
          <div className="grid grid-cols-4">
            ...
            specializations.map((specialization) => (
              <div key={specialization.id} className={`flex ${(specialization.id % 2 === 0) ? 'flex-col-reverse' : 'flex-col'}`}>
                ...
                <div className={`min-h-[250px] min-w-[250px] flex items-center justify-center font-sans text-xl font-bold uppercase text-white tracking-wider cursor-pointer ${(specialization.id % 2 === 0) ? 'bg-light_main_color' : 'bg-light_purple_color'}`}>{specialization.name}</div>
              </div>
            ))
          }
          </div>
